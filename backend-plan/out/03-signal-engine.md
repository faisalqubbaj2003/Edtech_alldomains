...
- Retention of the labels is the `fairness_label` class of pass 4 §7.1 (deleted once each run's report is accepted, and in any case within one year), and their erasure is independent of the student record's.
...
|---|---|---|---|
| `sweep.schedule` | cron, every 15 minutes | For each school returned by `core.schools_due_for_sweep(now())` (pass 1 §2.4: an ID-only definer function returning the schools that **hold data**, `core.school.status IN ('shadow','live')`, whose local time has passed `sweep_hour`, default 02:00, `core.school.sweep_hour`, **and whose nightly sweep has been enabled**, `core.school.sweep_enabled_at IS NOT NULL`, set by the caseload lead after the backfill sweep over the history import has been reviewed, pass 1 §2.3, pass 2 §9 step 15), and that has no nightly run for today, enqueue `sweep.run`; the worker then opens one `withTenant()` per school. Revised by pass 8 (F36): the first version said "each active school", a status that kept the pilot school, in `shadow` until G-LIVE, from being swept at all. Synthetic tenants are swept under the same rule; pass 1 DR-8 seeds them at `live` with `sweep_enabled_at` set | |
...
- **One evaluation per run per student per evaluated week.** A backfill writes one evaluation for each week it covers, in one run (§13.5), so the first version's `UNIQUE (school_id, sweep_run_id, student_id)` could not hold; the key is `UNIQUE NULLS NOT DISTINCT (school_id, sweep_run_id, student_id, backfill_of)`, which pass 1's DDL carries with `backfill_of` since pass 8 session 3 (D17), which keeps nightly and event runs at one evaluation per student (their `backfill_of` is NULL) and lets a backfill write one per week (F40). A batch that retries after a crash re-evaluates the students it had not written; a student already written is skipped.
...
- **History first.** At least twenty weeks of each student's own history are imported before the first shadow sweep (§5), so the full window exists from the first night. The nightly shadow sweep does not start at G-REAL: it starts when the caseload lead enables it (`core.school.sweep_enabled_at`, pass 1 §2.3, audited `SWEEP_ENABLED`) after the backfill sweep over the history import has been reviewed (pass 2 §9 steps 14 and 15), and nothing is swept nightly before rule-set v1 exists, and shadow does not spend its weeks in cold start (the first version started after "six weeks of weekly data", shorter than its own history minimum, F24).
...
**Going live** moves the school from `shadow` to `live` on the recorded G-LIVE sign-off (pass 1 §2.3's `core.school.g_live_signoff`: who signed, when, and the first live rule-set version whose note records the numbers, audited `G_LIVE_SIGNED_OFF`; pass 6). Pass 1's status trigger refuses `shadow → live` without it, and a school suspended from shadow can only return to shadow. Open shadow cases are not copied into the live queue, where they would appear with a history the counselors never saw; the first live run opens live cases from the current evaluation, each shown as "opened at go-live" with a link to its shadow history.
...
Reveal sessions at the end of **shadow weeks 4 and 8**, and each later month, per counselor, sixty minutes. Shadow weeks are counted from the first nightly shadow sweep, the first night after the caseload lead enables the sweep (§14.1; the reviewed backfill sweep before it is not counted): week 1 is the school week in which it ran, and the pilot agreement states this count (§12.17 of the pass 7 review; pass 6's overlay, §1.11, uses the same count and calls them shadow weeks 4 and 8).
...
The caseload lead activates the first live version when all of the following hold, the version note records the numbers, and the lead records the G-LIVE sign-off against that version (pass 1 §2.3, §14.1):
...
**Pass 6 (build sequence).** `packages/engine` as a pure library with the scenario suite (§15.1, now S1 to S26 and the Wellesmere list) as its acceptance test, built before any screen and before the seed's narrative layer is retired; `calibrate_tiers.py` (appendix) committed beside it and reproduced by B2.9; `engine-reproduces-seed` (B2.16) over both tenants and the CI-only shifted tenant against generated expected-result lists; migrations D17 to D32 in the phases of pass 6 §1.12 (D27, D28 and D30 with B0's global seeds, the rest in B2), with D17's new unique key; the Azure Monitor alert rules and the `/health/sweep` endpoint as part of the sweep's definition of done; the elicitation session (§10.5) scheduled before the first configuration version is written, including the G-LIVE floors; the history import before the first shadow sweep; shadow on the live engine with shadow cases (pass 6 §1.11; G-LIVE per §14.5 here); the backtest as a UAE North job (F23); the common-cause cap removed from B2.4 and O40 (done in pass 6's revision); Opus 5.5 writes and Fable 5.1 reviews the engine's statistics and tiering rows (C7, F38).
...
**Pass 1 (architecture).** D17 to D22 change tables pass 1 created, including D22's `triggering_state`; D17's unique key on `signal.evaluation` and its `backfill_of` column are now in pass 1's DDL (F40). Closed in pass 8 session 3: the synthetic tenants are seeded at `live` with the sweep enabled (pass 1 DR-8), the nightly sweep waits for `sweep_enabled_at`, and going live records `g_live_signoff` (pass 1 §2.3).
