...
| `ingest.after_commit` | batch | Recompute `normalised_pct` for touched grades; refresh `sis.school_day` for touched years; close cases for students who left (pass 1 C22); enqueue the backfill evaluation for every week the batch covers (§6.3; the engine applies the cap for the domain's cadence, pass 3 §4.8 as revised); enqueue `sweep.run` with trigger `event` for students whose facts changed if the school's policy allows daytime evaluation (pass 3); for committed `final` grades, mark the students for pass 3's `offer_condition_check` rule on results (F64; pass 3 decides whether it runs at once or in the next sweep); update `expected_cadence.last_as_of`; write `import.committed` | |
...
| 5 | The school's first `school_admin` (created in step 3) names a second, because every capability grant and every access-changing import needs two people (pass 4 §2.5; F16, F57); the two grant counselor memberships and the `caseload_lead` capability to the counseling lead, one requesting and the other confirming. CAROS grants nothing (F35) | S | memberships and `auth.capability_grant` rows |
...
| 14 | Thresholds and suppression configured with the counselors (ACS-IT-QUESTIONS.md §5) as `config.rule_set_version` v1 for the school, in shadow (the tenant has been `shadow` since step 7) | H with counselors | rule set versions |
...
| 19 | Shadow period ends when pass 3 §14.5's exit criteria hold as revised (pooled, pre-registered intervals; F24): counselors compare the engine's queue with their own judgement; thresholds adjusted as a new rule-set version; the demonstration marker rule is checked (`data_classification = 'real'` so the marker is off) | H + counselors | sign-off |
| 20 | **First live nightly sweep** (G-LIVE): `core.school.status` moves `shadow → live`; the run at the school's `sweep_hour` (default 02:00) succeeds; the `sweep.watchdog` at 05:45 local reports (§12.1 of the pass 7 review); the counselor sees "what changed overnight" at 07:00 | A, H watching | `signal.sweep_run` succeeded; onboarding closed |
...
| D8 | New table `config.grade_scale` mirroring `ref.grade_scale` with `school_id NOT NULL` for school-declared scales (`school:<slug>:<key>`), consulted before `ref.grade_scale` by the normaliser | `config` | §5.1 | B0 (pass 6's B1.1 also lists D8: disagreement) |
...
| D11 | `uni.document_request` gains `external_ref text` (Maia document id); `uni.offer` gains `external_ref text` | `uni` | §7.7 | B5 (pass 6's B1.1 also lists D11, before the `uni` tables exist: disagreement) |
...
| D13 | `doc.file.kind` gains `connector_snapshot` (JSON Lines from a live pull) and `sample_export` (pseudonymised onboarding sample), both in retention class `raw_import` | `doc` | §2.2, §9 | B1 (pass 6's B0 spine creates `doc.file` "with D13 kinds": disagreement) |
| D14 | `privacy.retention_class` gains `staged_row` (14 days after `failed`/`discarded`, 90 after `committed`, `delete`) | `privacy` | §2.2 | B0 (pass 6's B1.1 also lists D14: disagreement) |
| D15 | **Withdrawn as a row of its own by pass 8** (F16, §12.18). The first version added `('counselor','config','configure','school','school_admin')`, which made `school_admin` a counselor capability. It is now a role (pass 1 §2.4), and pass 4 §3.6's matrix gives that role `cf:S` on `config` (modules, mapping profiles, vocabularies, portal rules) and `r/w/ap:S` on `ingest`. Those rows are seeded with the matrix through pass 1 §2.4a; nothing of D15 is applied separately. The key is kept so pass 6's map does not shift | `auth` | §2.2, §2.11 | B0, as matrix rows (pass 6's B1.1 also lists D15: disagreement) |
